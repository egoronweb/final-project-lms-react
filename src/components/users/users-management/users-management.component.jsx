...
class UserManagement extends React.Component{
    ...
    async componentDidMount(){
        const resp = await axios.get('http://127.0.0.1:8000/api/admin-dashboard/users-management');
    ...
    deleteStudent = async (e, id) =>{
        const resp = await axios.delete(`http://127.0.0.1:8000/api/admin-dashboard/users-management/delete/${id}`);
        if(resp.data.status === 200){
            swal({
                title: "Deleting User",
              ...
              .then((willDelete) => {
                if (willDelete) {
                  swal("User Deleted Successfully!", "Please refresh the page", {
                    icon: "success",
                  });
                } else {
            ...
            userTable = this.state.users.map((item) => {
                return(
                    <tr key={item.id}>
                        ...
                        <td>{item.email}</td>
                        <td>
                            <button className="btn btn-success btn-sm"><a href={`users-management/edit/${item.id}`}>Edit</a></button>
                        </td>
                        <td>
            ...
            <div className="wrapper">
                ...
                <div className="content">
                    <div className="users-management-list">
                        <div className="title">Students Management</div>
                        <table className="user-table-management">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Full Name</th>
