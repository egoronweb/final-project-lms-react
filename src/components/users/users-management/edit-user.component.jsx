...
class EditUser extends React.Component{
    ...
    async componentDidMount() {
        ...
        const resp = await axios.get(`http://127.0.0.1:8000/api/admin-dashboard/users-management/edit/${user_id}`);
    ...
    updateUser = async (e) => {
        ...
        const user_id = this.props.match.params.id;
                ...
                const resp = await axios.put(`http://127.0.0.1:8000/api/admin-dashboard/users-management/edit/update/${user_id}`, this.state);
                if(resp.data.status === 200){
                    swal("User Updated Successfully!", "Redirect?", "success");
        ...
        return(
           ...
           <nav className="top-nav-bar">
            ...
            <div className="content">
                ...
                <form onSubmit={this.updateUser} className="edit-form">
                        <div className="mb-3">
                            <label htmlFor="fullname" className="form-label">Full Name</label>
                            <input type="text" name="fullname" onChange={e =>
                            this.setState({fullname:e.target.value})}
                                value={this.state.fullname} className="form-control" id="fullname" required/>
                        </div>
                        <div className="mb-3">
                            <label htmlFor="email" className="form-label">Email address</label>
                            <input type="email" name="email" onChange={e => 
                                {this.setState({email:e.target.value})}} 
                                value={this.state.email} className="form-control" id="email" required/>
                        </div>
                        <div className="mb-3">
                            <label htmlFor="password" className="form-label">Password</label>
                            <input type="password" name="password" onChange={e => 
                                {this.setState({password:e.target.value})}} 
                                value={this.state.password} className="form-control" id="password" required/>
                        </div>
            ...
            <footer className="footer">
                ...
                <div className="footer-center">
                    ...
                    <p className="footer-link">egoron.info</p>
                </div>
