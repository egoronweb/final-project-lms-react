import React from "react";
import swal from "sweetalert";
import { Redirect } from "react-router-dom";


import '../../lms-styling/lms-styling.scss';
class AdminLogin extends React.Component{
constructor() {
    super();

    this.state = {
        id: 1,
        email: '',
        password: '',
    }
}
    loginAdmin = (e) => {
        e.preventDefault();
        const email = 'admin@egoron.info';
        const password = 'admin';



        if(email === this.state.email & password === this.state.password){
            console.log('Credentails matched');
            swal({
                title: "Credentials Matched",
                text: "Redirect?",
                icon: "success",
                button: "OK",
              });
              return <Redirect to="/admin-dashboard" />
        }else{
            console.log('Credentials not match');
            swal("Credentials not Match", "Login again!", "error");
            return <Redirect to="/admin-login" />
        }
    }

    render(){
        return(
            <div className="wrapper">
                <nav className="top-nav-bar">
                    <div className="mylogo"><a href="/">egoron<span>Web</span></a></div>
                    <ul className="menu-texts">
                        <li><a href="/">Home</a></li>
                        <li><a href="/about">About</a></li>
                        <li><a href="/contact-us">Contact us</a></li>
                        <li><a href="/user-register">Sign Up</a></li>
                        <li><a href="/user-login" className="active">Login</a></li>
                    </ul>
                </nav>


                <div className="content">
                    <div className="content-form">
                        <form className="login-form">
                            <div className="form-btn-options">
                                <a href="/user-login"><button type="button" className="guest-option-btn">Guest</button></a>
                                <a href="/admin-login"><button type="button" className="admin-option-btn active">Admin</button></a>
                            </div>
                            <div className="login-title">Login as <span>Admin</span></div>
                            <div className="mb-3">
                                <label htmlFor="admin_email" className="form-label">Email address</label>
                                <input type="email" className="form-control" id="admin_email" onChange={(e) => {
                                this.setState({email:e.target.value})}} required/>
                            </div>
                            <div className="mb-3">
                                <label htmlFor="admin_password" className="form-label">Password</label>
                                <input type="password" className="form-control" id="admin_password" onChange={e => {
                                this.setState({password:e.target.value})}} required/>
                            </div>
                            <button type="submit" className="btn btn-primary" onClick={this.loginAdmin}>Login</button>
                        </form>
                    </div>
                </div>


                <footer className="footer">
                    <div className="footer-left">
                        <span>Privacy and Policy</span>
                        <span>Terms and Conditions</span>
                        <span>Contact us</span>
                    </div>
                    <div className="footer-center">
                        <p>Alright Reserved@2022</p>
                        <p>egoron.info</p>
                    </div>
                    <div className="footer-right">
                        <img src="https://cdn.pixabay.com/photo/2021/08/16/08/55/facebook-6549798_1280.jpg" alt="" />
                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e7/Instagram_logo_2016.svg/800px-Instagram_logo_2016.svg.png" alt="" />
                        <img src="data:image/png;base64," alt="" />
                        <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTXimNscwCPD5sHXBMBBOUIhxS4D_txKTuqQw&usqp=CAU" alt="" />
                    </div>
                </footer>

            </div>
        );
    }
}

export default AdminLogin;